...
// --- Helper Functions ---
const handleNotImplemented = (req, res) => res.status(501).json({ message: 'Not Implemented' });
...
// --- /api/quizzes ---
// Membuat kuis baru
const createQuiz = async (req, res) => { // POST
    const { title, description, submissionStartDate, deadline, duration } = req.body;
    const authorId = req.user.id;

    if (!title || !deadline) {
        return res.status(400).json({ message: "Judul dan deadline kuis dibutuhkan." });
    }
    ...
    if (submissionStartDate) {
        ...
        if (isNaN(ssd.getTime())) {
            return res.status(400).json({ message: "Format tanggal mulai pengerjaan tidak valid." });
        }
    }
    if (isNaN(dl.getTime())) {
    ...
    if (ssd && ssd >= dl) {
        return res.status(400).json({ message: "Tanggal mulai pengerjaan harus sebelum deadline." });
    }
...
const getAllQuizzesAdmin = async (req, res) => { // GET
    try {
...
const getQuizByIdAdmin = async (req, res) => { // GET
    ...
    try {
        ...
        res.status(500).json({ message: "Gagal mengambil detail kuis.", error: error.message });
    }
};

// Mengupdate kuis
const updateQuiz = async (req, res) => { // PUT
    const { quizId } = req.params;
    ...
    try {
        ...
        if (quizToUpdate.authorId !== userId && req.user.role !== 'ADMIN') {
            return res.status(403).json({ message: "Anda tidak berhak mengubah kuis ini." });
        }
        ...
        let newStartDate = quizToUpdate.submissionStartDate, newEndDate = quizToUpdate.deadline;

        if (submissionStartDate) {
            newStartDate = new Date(submissionStartDate);
            if (isNaN(newStartDate.getTime())) return res.status(400).json({ message: "Format tanggal mulai tidak valid." });
        }
        if (deadline) {
            newEndDate = new Date(deadline);
            if (isNaN(newEndDate.getTime())) return res.status(400).json({ message: "Format deadline tidak valid." });
        }
        if (newStartDate && newEndDate && newStartDate >= newEndDate) { // Periksa hanya jika keduanya ada
            return res.status(400).json({ message: "Tanggal mulai pengerjaan harus sebelum deadline." });
        }
        if(submissionStartDate !== undefined) dataToUpdate.submissionStartDate = newStartDate; // Update jika ada di body
        ...
        res.status(500).json({ message: "Gagal memperbarui kuis.", error: error.message });
    }
};
const patchQuiz = async (req, res) => { // PATCH
    ...
    try {
        const quizToUpdate = await prisma.quiz.findUnique({ where: { id: quizId }});
        if (!quizToUpdate) return res.status(404).json({ message: "Kuis tidak ditemukan." });
        if (quizToUpdate.authorId !== userId && req.user.role !== 'ADMIN') {
            return res.status(403).json({ message: "Anda tidak berhak mengubah kuis ini." });
        }
        ...
        const finalStartDate = updateData.submissionStartDate || quizToUpdate.submissionStartDate;
        ...
        res.status(500).json({ message: "Gagal melakukan patch pada kuis.", error: error.message });
    }
};


// Menghapus kuis (beserta semua soal dan percobaan terkait karena onDelete: Cascade)
const deleteQuiz = async (req, res) => { // DELETE
    const { quizId } = req.params;
    const userId = req.user.id;
    try {
        ...
        // Prisma akan menghapus Question dan QuizAttempt terkait jika onDelete: Cascade diatur di schema.prisma
        // Jika QuizAnswer juga perlu dihapus saat Question dihapus, atur onDelete: Cascade di relasi Question-QuizAnswer
        await prisma.quiz.delete({ where: { id: quizId } });
        res.json({ message: "Kuis berhasil dihapus." });
    } catch (error) {
        console.error("Delete quiz error:", error);
        // Periksa error spesifik jika ada, misal P2003 jika ada foreign key constraint yang menghalangi
...
const addQuestionToQuiz = async (req, res) => { // POST
    const { quizId } = req.params;
    const { text, options,type,imageUrl } = req.body; // options: [{"text": "A", "isCorrect": true}, ...]
    const userId = req.user.id;

    if (!text || !options) {
        return res.status(400).json({ message: "Teks soal dan pilihan jawaban (options) dibutuhkan." });
    }
    ...
    try {
        ...
        if (quiz.authorId !== userId && req.user.role !== 'ADMIN') {
            return res.status(403).json({ message: "Anda tidak berhak menambahkan soal ke kuis ini." });
        }
        ...
        res.status(201).json({ message: "Soal berhasil ditambahkan ke kuis.", question });
    } catch (error) {
...
const getQuestionsForQuizAdmin = async (req, res) => { // GET
    const { quizId } = req.params;
    try {
        const quiz = await prisma.quiz.findUnique({ where: { id: quizId }});
        if (!quiz) {
...
const updateQuestion = async (req, res) => {
  ...
  // FIX: Tambahkan 'imageUrl' di sini juga
  const { text, type, options, imageUrl } = req.body;

  // ... (validasi sama seperti di atas)
...
const patchQuestion = async (req, res) => { // PATCH
...
const deleteQuestion = async (req, res) => { // DELETE
    ...
    try {
        ...
        if (!question || question.quizId !== quizId) {
            return res.status(404).json({ message: "Soal tidak ditemukan pada kuis ini." });
        }
        ...
        // Hapus jawaban terkait soal ini jika onDelete: Cascade tidak diatur di QuizAnswer-Question
        // await prisma.quizAnswer.deleteMany({ where: { questionId: questionId }}); 
...
const getAvailableQuizzesForStudent = async (req, res) => { // GET
    const now = new Date();
    try {
        const quizzes = await prisma.quiz.findMany({
            where: {
            ...
            select: { // Pilih field yang relevan untuk siswa
                id: true,
                title: true,
                description: true,
...
const getQuizForStudentToTake = async (req, res) => { // GET
    const { quizId } = req.params;
    ...
    try {
        ...
        if (existingAttempt) {
            return res.status(403).json({ message: "Anda sudah pernah mengerjakan kuis ini.", attemptId: existingAttempt.id });
        }
        ...
        const quizFromDb = await prisma.quiz.findUnique({
            ...
            include: {
                questions: {
                    ...
                    select: { // Hanya pilih field yang dibutuhkan siswa
                        id: true,
                        text: true,
        ...
        const isQuizActive = (quizFromDb.submissionStartDate ? now >= quizFromDb.submissionStartDate : true) && now <= quizFromDb.deadline;
        if (!isQuizActive) {
            return res.status(403).json({ message: "Kuis ini tidak aktif atau sudah melewati batas waktu." });
        }
        
        // FIX 2: Modifikasi options dengan aman dan hapus 'isCorrect'
        const questionsForStudent = quizFromDb.questions.map(q => {
            let optionsForStudent = []; // Default ke array kosong

            // Hanya proses 'options' jika ada dan merupakan array
        ...
        res.status(500).json({ message: "Gagal mengambil detail kuis untuk dikerjakan.", error: error.message });
    }
};
...
const submitQuizAttempt = async (req, res) => {
  const { quizId } = req.params;
  const studentId = req.user.id;
  const { answers } = req.body; // answers: [{ questionId, selectedOptionIndex?, answerText? }]
  ...
  try {
    ...
    // FIX: Baris validasi yang mengharuskan semua jawaban diisi telah DIHAPUS.
    // if (quiz.questions.length !== answers.length) {
    //   return res.status(400).json({ message: "Harap jawab semua pertanyaan." });
    // }
    ...
    for (const userAnswer of answers) {
      ...
      if (
        ...
        // Jika jawaban tidak ada (null), lewati pengecekan
        if (
          ...
          answerData.selectedOptionIndex = userAnswer.selectedOptionIndex;
        }
      } else if (question.type === "ESSAY") {
        if (userAnswer.answerText && userAnswer.answerText.trim() !== "") {
          answerData.answerText = userAnswer.answerText;
        }
        // Penilaian esai manual, jadi isCorrect default false
      }
    ...
    const newAttempt = await prisma.quizAttempt.create({
      data: {
        ...
        answers: { create: quizAnswersData },
      },
    });
...
// Siswa melihat hasil percobaan kuis mereka
const getMyQuizAttemptResult = async (req, res) => { // GET
    const { quizId } = req.params;
    const studentId = req.user.id;
    ...
    try {
        ...
        const attempt = await prisma.quizAttempt.findUnique({
            ...
            include: {
                ...
                answers: {
                    include: {
                        question: { 
                            select: { id: true, text: true, options: true, type: true, imageUrl: true }
                        }
                    },
        ...
        if (!attempt) {
            return res.status(404).json({ message: "Anda belum mengerjakan kuis ini atau hasil tidak ditemukan." });
        }
        
        // FIX: Hapus pemrosesan 'detailedAnswers' yang rentan error.
        // Kirim objek 'attempt' yang diterima langsung dari Prisma.
        // Frontend sudah dirancang untuk menangani struktur data ini.
        res.json({ message: "Berhasil mengambil hasil kuis", data: attempt });

    } catch (error) {
        console.error("Get my quiz attempt result error:", error);
        res.status(500).json({ message: "Gagal mengambil hasil kuis Anda.", error: error.message });
    }
};
